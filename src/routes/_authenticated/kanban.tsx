...
function KanbanPage() {
  ...
  const createBoard = async (name: string) => {
    ...
    // Default columns (Trello style)
    await supabase.from("kanban_categories").insert([
      { user_id: user.id, board_id: data.id, name: "A Fazer", color: "#94a3b8", order: 1 },
      { user_id: user.id, board_id: data.id, name: "Em Andamento", color: "#3b82f6", order: 2 },
      { user_id: user.id, board_id: data.id, name: "Alteração", color: "#f59e0b", order: 3 },
      { user_id: user.id, board_id: data.id, name: "Finalizado", color: "#10b981", order: 4 },
    ]);
  ...
  return (
    <div className="space-y-6 h-full flex flex-col">
      {!activeBoardId ? (
        <>
          <div>
            <h1 className="text-4xl font-extrabold tracking-tight text-[#172B4D] dark:text-foreground">Planejamentos</h1>
            <p className="text-muted-foreground mt-1 text-base font-medium opacity-80">Organize suas metas e fluxos de trabalho no estilo Trello.</p>
          </div>
          <QuadrosList 
            boards={boards} 
